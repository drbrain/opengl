...
* There are several of the convenience wrapper functions that Yoshi wrote
...
The directory structure is:
    ...
    lib/
        Ruby files that provide glue code to run programs written in the
        naming style of Yoshi's code.
    test/
...
SWIG[http://www.swig.org/] must be installed on the build machine.  I'm
using the fink version of swig on my mac:
...
To build, just run <tt>rake</tt> at the top level.  This will do everything
necessary to build both the OpenGL and GLUT bindings for ruby.  The top
level +rakefile+ calls down to <tt>ext/gl/rakefile</tt> and
<tt>ext/glut/rakefile</tt> to do the real work.
...
Just hit <tt>ESC</tt> to exit the super-interesting OpenGL program.
